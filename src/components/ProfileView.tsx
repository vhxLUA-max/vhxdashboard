...
const SOCIAL_BADGES: { key: keyof UserSocials; label: string; color: string; bg: string; icon: React.ReactNode }[] = [
  {
    key: 'discord', label: 'Discord', color: '#5865F2', bg: 'rgba(88,101,242,0.18)',
    icon: <svg viewBox="0 0 127.14 96.36" fill="currentColor" className="w-3 h-3"><path d="M107.7 8.07A105.15 105.15 0 0 0 81.47 0a72.06 72.06 0 0 0-3.36 6.83 97.68 97.68 0 0 0-29.11 0A72.37 72.37 0 0 0 45.64 0a105.89 105.89 0 0 0-26.25 8.09C2.79 32.65-1.71 56.6.54 80.21a105.73 105.73 0 0 0 32.17 16.15 77.7 77.7 0 0 0 6.89-11.11 68.42 68.42 0 0 1-10.85-5.18c.91-.66 1.8-1.34 2.66-2a75.57 75.57 0 0 0 64.32 0c.87.71 1.76 1.39 2.66 2a68.68 68.68 0 0 1-10.87 5.19 77 77 0 0 0 6.89 11.1 105.25 105.25 0 0 0 32.19-16.14c2.64-27.38-4.51-51.11-18.9-72.15zM42.45 65.69C36.18 65.69 31 60 31 53s5-12.74 11.43-12.74S54 46 53.89 53s-5.05 12.69-11.44 12.69zm42.24 0C78.41 65.69 73.25 60 73.25 53s5-12.74 11.44-12.74S96.23 46 96.12 53s-5.04 12.69-11.43 12.69z"/></svg>
  },
  {
    key: 'youtube', label: 'YouTube', color: '#FF0000', bg: 'rgba(255,0,0,0.15)',
  ...
  {
    key: 'tiktok', label: 'TikTok', color: '#ff0050', bg: 'rgba(255,0,80,0.15)',
    icon: <svg viewBox="0 0 24 24" fill="currentColor" className="w-3 h-3"><path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-2.88 2.5 2.89 2.89 0 0 1-2.89-2.89 2.89 2.89 0 0 1 2.89-2.89c.28 0 .54.04.79.1V9.01a6.27 6.27 0 0 0-.79-.05 6.34 6.34 0 0 0-6.34 6.34 6.34 6.34 0 0 0 6.34 6.34 6.34 6.34 0 0 0 6.33-6.34V8.69a8.18 8.18 0 0 0 4.78 1.52V6.75a4.85 4.85 0 0 1-1.01-.06z"/></svg>
  },
  {
    key: 'twitter', label: 'X', color: '#e7e9ea', bg: 'rgba(231,233,234,0.1)',
    icon: <svg viewBox="0 0 24 24" fill="currentColor" className="w-3 h-3"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.742l7.776-8.896-8.208-10.604h7.242l4.26 5.633zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
  },
  {
    key: 'github', label: 'GitHub', color: '#e6edf3', bg: 'rgba(230,237,243,0.1)',
  ...
  {
    key: 'twitch', label: 'Twitch', color: '#9147ff', bg: 'rgba(145,71,255,0.15)',
    icon: <svg viewBox="0 0 24 24" fill="currentColor" className="w-3 h-3"><path d="M11.571 4.714h1.715v5.143H11.57zm4.715 0H18v5.143h-1.714zM6 0L1.714 4.286v15.428h5.143V24l4.286-4.286h3.428L22.286 12V0zm14.571 11.143l-3.428 3.428h-3.429l-3 3v-3H6.857V1.714h13.714z"/></svg>
  },
];
...
export function ProfileView({ username, avatarUrl, isAdmin, isLoggedIn, onEditProfile, compact = false }: ProfileViewProps) {
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-0.5">
                ...
                <span className="text-[10px]" style={{ color: 'rgba(148,163,184,0.6)' }}>· Online</span>
              </div>
            </div>
          </div>
  ...
  return (
    <div className="w-full overflow-hidden" style={{ fontFamily: "'Inter', sans-serif" }}>
      {/* Banner */}
      <div className="relative w-full overflow-hidden" style={{ height: bannerUrl ? '140px' : '0' }}>
      ...
      <div className="px-4 pt-3 pb-4">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {isOnline && (
            <span className="text-[13px] flex items-center gap-1.5" style={{ color: '#10b981' }}>
        ...
        {bio && (
          <p className="text-[13px] mt-2.5 leading-relaxed" style={{ color: '#94a3b8' }}>{bio}</p>
        )}
      </div>
    </div>
  );
